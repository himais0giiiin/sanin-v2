document.addEventListener('DOMContentLoaded', async () => {
    ...
    const announcements = [
        ...
        {id: 'stop1', type: 'info', message:'広島・山口地区(山陰本線、山陽線)では、明日（７月１８日）の早朝から大雨が見込まれています。このため、列車の遅れや運転取り止め、急遽の行先変更がでる可能性があります。※運転を取り止めた場合、代行輸送は行いません。※その他の線区においても、列車の遅れや運転取り止め、急遽の行先変更がでる可能性があります。'},
        ];
    ...
    function renderStations() {
        ...
        stations.forEach(station => {
            ...
            if (station.iconUrl) {
                ...
                // 駅アイコンにツールチップ表示機能を追加
                const tooltipContent = `<b>駅名:</b> ${station.name}<br><b>乗入路線:</b> ${station.stationCode || station.id}`;
                ...
                if (isMobileDevice()) {
                    ...
                    iconWrapperDiv.addEventListener('mouseover', (e) => {
                        showTooltip(e.target, tooltipContent);
                    });
                    iconWrapperDiv.addEventListener('mouseout', hideTooltip);
                }
            }
    ...
    document.addEventListener('click', (e) => {
        // ツールチップ、列車アイコン、運休区間、駅アイコン以外の場所がクリックされたら非表示
        if (!e.target.closest('.tooltip') && !e.target.closest('.train') && !e.target.closest('.suspended-section') && !e.target.closest('.station-icon-wrapper')) {
            hideTooltip();
        }
    });
    ...
    function updatePositions() {
        ...
        trainSchedules.forEach(train => {
            ...
            if (issue) {
                ...
                // イベントリスナーの追加 (PCはホバー、スマホはタップ)
                if (isMobileDevice()) {
                    trainDiv.addEventListener('click', (e) => { e.stopPropagation(); showTooltip(e.target, tooltipContent); });
                } else {
                    trainDiv.addEventListener('mouseover', (e) => { showTooltip(e.target, tooltipContent); });
                    trainDiv.addEventListener('mouseout', hideTooltip);
                }
            ...
            for (let i = 0; i < train.timetable.length; i++) {
                ...
                if (i === train.timetable.length - 1 && arrivalTime && train.type !== '代行輸送') {
                    ...
                    if (now >= arrivalTime && now <= oneMinuteAfterArrival) {
                        ...
                        trainDiv.style.top = `${trainY - 12.5}px`;
                        const tooltipContent = `<b>種別:</b> ${train.type}<br><b>列車番号:</b> ${train.trainId}<br><b>状況:</b> 到着済`;
                ...
                if (i === 0 && departureTime && train.type !== '代行輸送') {
                    ...
                    if (now >= threeMinutesBeforeDeparture && now <= departureTime) {
                        ...
                        trainDiv.innerHTML = `<img src="${imagePaths.waiting}" alt="停車中(定刻)" style="width: 25px; height: 25px;">`;
                        trainDiv.style.top = `${trainY - 12.5}px`;
                        const tooltipContent = `<b>種別:</b> ${train.type}<br><b>列車番号:</b> ${train.trainId}<br><b>状況:</b> 発車待機中`;
                    ...
                    if (nextArrivalTime && trainTime >= departureTime && trainTime < nextArrivalTime) {
                        ...
                        if (train.type === '代行輸送') {
                            ...
                            if (train.delayMinutes > 0) {
                                statusText = `${train.delayMinutes}分遅延`;
                                ...
                                statusText = `${Math.abs(train.delayMinutes)}分早発`;
                        ...
                        trainDiv.innerHTML = `<img src="${imgSrc}" alt="${altText}" style="width: 25px; height: 25px;">`;
                        trainDiv.style.top = `${trainY - 12.5}px`;
                        const tooltipContent = `<b>種別:</b> ${train.type}<br><b>列車番号:</b> ${train.trainId}<br><b>行先:</b> ${train.destination}<br><b>状況:</b> ${statusText}`;
